...
msg_outtrans_len_attr_regex(char_u *msgstr, int len, int attr, int regex)
{
    ...
    int		attr3 = attr;
    int		whacked = 0;
    int		state = 'P'; // Prior, Normal, Bracketted, Slashed
    int		mb_l;
    int		c;
    ...
    // We need to know if we're in a search [/?] or regular [:] cmdline mode
    cmdline_info_T	*ccline = get_cmdline_info();

    // Jump directly to Normal state if this is a normal search
	...
	else
	{
		...
		plain_start = str + consumed;
		    ...
		    attr3 = HL_ATTR(HLF_8);
		else if (strchr("nmlr/[]oS", tokentype))
		...
		if (state == 'N' && tokentype == '[') // opening [ or \_[
		    ...
		    state = 'N';
		else if (state == 'P' && tokentype == 'S') // start of search
		    state = 'N';
	    }
